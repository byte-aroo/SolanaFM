...
export default function DownloadSolanaByHash() {
  ...
  const handleDownloadClick = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {

    event.preventDefault();

    if (!hash) {
      setDownloadMessage("Hash is required.");
      setTimeout(() => {
        setDownloadMessage("");
      }, 5000);
      return;
    ...
    if (utcFrom && (!isValidEpoch(utcFrom) || Number(utcFrom) > currentTime)) {
      setDownloadMessage("utcFrom must be a valid epoch time & less than the current time.");
      setTimeout(() => {
        setDownloadMessage("");
      }, 5000);
      return;
    ...
    if (utcTo && (!isValidEpoch(utcTo) || Number(utcTo) > currentTime)) {
      setDownloadMessage("utcTo must be a valid epoch time less than the current time.");
      setTimeout(() => {
        setDownloadMessage("");
      }, 5000);
      return;
    ...
    try {
      ...
      let url = `https://api.solana.fm/v0/accounts/${hash}/transfers/csv?`;
      if(utcFrom || utcTo){
        if(utcFrom) {
          url += `utcFrom=${utcFrom}&`;
        }
      ...
      if(!response.ok){
        ...
        const errorMssg = errorResponse.message || response.statusText;
        setDownloadMessage(`API response was not OK: ${errorMssg}`)
        return;
      }
      
      const result = await response.text();
      if(!result.trim()){
        setDownloadMessage("No records found.");
        return;
      }
      ...
      a.href = urlObject
      a.download = `transfers_${currentTime}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setDownloadMessage("CSV will be downloaded soon");
    } catch (error) {
  ...
  const isValidEpoch = (epoch: string): boolean => {
    const epochNumber = Number(epoch);
    return !isNaN(epochNumber) && epochNumber > 0 && new Date(epochNumber * 1000).getTime() > 0;
  }
        ...
        <div className="relative bg-purple-600 py-10 px-8 md:py-16 md:px-12" data-aos="fade-up">
          ...
          <div className="relative flex flex-col lg:flex-row justify-between items-center">
            ...
            <div className="mb-6 lg:mr-16 lg:mb-0 text-center lg:text-left lg:w-1/2">
              <h3 className="h3 text-white mb-2">SolanaFM</h3>
              <p className="text-purple-200 text-lg">Accounts: Transfer CSV Export</p>
            </div>
            ...
            <form className="max-w-xs mx-auto">
              <div className="mb-4">
                <input
                  type="text"
                  className="w-full appearance-none bg-purple-700 border border-purple-500 focus:border-purple-300 rounded-sm px-4 py-3 mb-2 text-white placeholder-purple-400"
                  placeholder="hash: Account pubkey"
                  aria-label="Enter hash"
                  value={hash}
                ...
                <input
                  type="text"
                  className="w-full appearance-none bg-purple-700 border border-purple-500 focus:border-purple-300 rounded-sm px-4 py-3 mb-2 text-white placeholder-purple-400"
                  placeholder="utcFrom: in epoch (optional)"
                  aria-label="utcFrom"
                  value={utcFrom}
                  onChange={(e) => setUtcFrom(e.target.value)}
                />
                <input
                  type="text"
                  className="w-full appearance-none bg-purple-700 border border-purple-500 focus:border-purple-300 rounded-sm px-4 py-3 mb-2 text-white placeholder-purple-400"
                  placeholder="utcTo: in epoch (optional)"
                  aria-label="utcTo"
                  value={utcTo}
                  onChange={(e) => setUtcTo(e.target.value)}
                />
              </div>
              <div className="text-center">
                <button className="btn text-purple-600 bg-purple-100 hover:bg-white shadow px-6 py-3" onClick={handleDownloadClick}>Download CSV</button>
              </div>
              {downloadMessage && (
                <p className="text-center text-gray-500">{downloadMessage}</p>
              )}
            </form>
